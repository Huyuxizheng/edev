...
inline void ev_dri_i2c_init(EV_DRI_S(I2C) *i2c){
    if(i2c->init)
        i2c->init(i2c->handle);
}
inline uint8_t ev_dri_i2c_write(EV_DRI_S(I2C) *i2c,uint8_t addr,uint8_t *dat,uint16_t len){
    if(i2c->write)
        return i2c->write(i2c->handle,addr,dat,len);
    return 0;
}
inline uint8_t ev_dri_i2c_read(EV_DRI_S(I2C) *i2c,uint8_t addr,uint8_t *dat,uint16_t len){
    if(i2c->read)
        return i2c->read(i2c->handle,addr,dat,len);
    return 0;
}
inline void ev_dri_i2c_uninit(EV_DRI_S(I2C) *i2c){
    if(i2c->uninit)
